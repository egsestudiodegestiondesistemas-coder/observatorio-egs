...
st.set_page_config(page_title="EGS | Observatorio Territorial", page_icon="🌱", layout="wide")
...
def inject_css():
    ...
    .egs-header {{
        ...
        border-radius: 24px;
        margin-bottom: 28px;
        box-shadow: 0 8px 20px rgba(19,54,47,0.12);
    }}
    .egs-header h1 {{ font-size: 44px; margin-bottom: 8px; letter-spacing: -0.5px; }}
    .egs-header p {{ font-size: 17px; color: #E7F0EC; }}
    .metric-card {{
        background: white;
        ...
        padding: 22px;
        min-height: 155px;
        box-shadow: 0 4px 12px rgba(19,54,47,0.04);
    }}
    .metric-card h4 {{ color: {EGS_GREEN}; font-size: 16px; margin-bottom: 14px; }}
    .metric-card h2 {{ font-size: 34px; margin: 0; color: #17202A; }}
    .metric-card p {{ color: {EGS_GRAY}; font-size: 14px; }}
    .alert-box {{
        background: white;
...
def login_screen():
    ...
    <div class="egs-header">
        <h1>EGS | Observatorio Territorial</h1>
        <p>Acceso interno · Análisis territorial · Gestión ambiental · Indicadores estratégicos</p>
    </div>
    """, unsafe_allow_html=True)
...
def demo_data():
    return pd.DataFrame([
        ["2026-01-10", "Baches", "Bache en esquina", "Av. Libertador 1200", "Centro", -31.4275, -62.0821, "Pendiente", "Media", "", 6],
        ["2026-01-18", "Iluminación", "Luminaria apagada", "Bv. 9 de Julio 850", "Centro", -31.4301, -62.0812, "Resuelto", "Alta", "2026-01-25", 7],
        ["2026-02-03", "Pérdida de agua", "Pérdida constante", "Córdoba 300", "Sarmiento", -31.4259, -62.0910, "En proceso", "Alta", "", 9],
        ["2026-02-20", "Arbolado", "Rama con riesgo", "Mitre 600", "Roca", -31.4210, -62.0844, "Pendiente", "Alta", "", 8],
        ["2026-03-14", "Residuos", "Acumulación de residuos", "Italia 950", "San Cayetano", -31.4330, -62.0744, "Resuelto", "Media", "2026-03-20", 6],
        ["2026-03-21", "Drenaje urbano", "Calle anegada", "Av. Rosario 1800", "Independencia", -31.4365, -62.0895, "Pendiente", "Alta", "", 9],
        ["2026-04-04", "Veredas", "Vereda rota", "Belgrano 700", "Centro", -31.4282, -62.0799, "Pendiente", "Baja", "", 4],
        ["2026-04-27", "Baches", "Pozo grande", "Urquiza 1100", "Roca", -31.4225, -62.0878, "En proceso", "Media", "", 7],
        ["2026-09-08", "Iluminación", "Zona oscura", "Avellaneda 1400", "Sarmiento", -31.4267, -62.0932, "Resuelto", "Alta", "2026-09-18", 8],
        ["2026-10-02", "Pérdida de agua", "Pérdida sobre calzada", "San Martín 400", "Centro", -31.4279, -62.0804, "Pendiente", "Alta", "", 10],
    ], columns=["fecha", "categoria", "descripcion", "direccion", "barrio", "latitud", "longitud", "estado", "prioridad", "fecha_resolucion", "criticidad"])
...
def normalize_columns(df):
    mapping = {
        "fecha": ["fecha", "date", "created", "timestamp", "fecha_reclamo", "fecha de carga"],
        "categoria": ["categoria", "categoría", "tipo", "tipo_reclamo", "reclamo", "problema"],
        "descripcion": ["descripcion", "descripción", "detalle", "observacion", "observación", "comentario"],
    ...
    pesos = {"Pérdida de agua": 9, "Perdida de agua": 9, "Drenaje urbano": 9, "Arbolado": 8, "Iluminación": 7, "Iluminacion": 7, "Baches": 6, "Residuos": 6, "Veredas": 4}
    result["criticidad"] = result.apply(lambda r: pesos.get(str(r["categoria"]).strip(), 5) if pd.isna(r["criticidad"]) else r["criticidad"], axis=1)
    result["mes"] = result["fecha"].dt.to_period("M").astype(str)
...
def territorial_ranking(df):
    ...
    ranking["indice_egs"] = (ranking["incidencias"] * 1.5 + ranking["criticidad_promedio"] * 2 + ranking["casos_activos"] * 1.4).round(1)
    ranking["nivel_prioridad"] = ranking["indice_egs"].apply(lambda x: "Prioridad 1 - Crítica" if x >= 25 else "Prioridad 2 - Media" if x >= 15 else "Prioridad 3 - Baja")
...
def generar_informe_tecnico(df, selected_months):
    total = len(df)
    ...
    prioridad_1 = len(ranking[ranking["nivel_prioridad"] == "Prioridad 1 - Crítica"]) if not ranking.empty else 0
    ...
    <head>
    <meta charset="utf-8">
    <title>Informe Técnico EGS</title>
    <style>
        ...
        h2 {{ color: #13362F; border-bottom: 2px solid #D8E7B8; padding-bottom: 6px; }}
        table {{ width: 100%; border-collapse: collapse; margin: 16px 0; }}
        th, td {{ border: 1px solid #DDE5E0; padding: 9px; text-align: left; }}
    ...
    <body>
        <div class="cover">
            <h1>EGS | Informe Técnico Territorial</h1>
            <p class="sub">Estudio de Gestión de Sistemas · Análisis territorial · Gestión ambiental · Indicadores estratégicos</p>
        ...
        <div class="metric"><p>Prioridad media EGS</p><b>{criticidad}</b></div>

        <h2>2. Alcance técnico</h2>
        <p>El presente informe consolida registros de incidencias urbanas cargadas en formato CSV o Excel y procesadas mediante el Observatorio Territorial EGS. El análisis integra variables de categoría, estado, localización, barrio, criticidad, fecha de registro y fecha de resolución cuando se encuentra disponible.</p>

        <h2>3. Resultados principales</h2>
        <p>La categoría predominante fue <b>{cat_top}</b>. El sector con mayor concentración territorial fue <b>{barrio_top}</b>. Se identificaron <b>{prioridad_1}</b> sectores clasificados como Prioridad 1 - Crítica según el Índice Territorial EGS.</p>

        <h2>4. Metodología del Índice Territorial EGS</h2>
        <p>El Índice Territorial EGS combina tres dimensiones: cantidad de incidencias, criticidad promedio y casos activos. La ponderación permite priorizar sectores donde coinciden recurrencia, gravedad técnica y falta de resolución.</p>
        <div class="note"><b>Fórmula aplicada:</b> Índice EGS = incidencias × 1,5 + criticidad promedio × 2 + casos activos × 1,4.</div>

        <h2>5. Ranking territorial</h2>
        {ranking.head(15).to_html(index=False) if not ranking.empty else "<p>Sin datos suficientes para ranking territorial.</p>"}

        <h2>6. Lectura técnica</h2>
        <p>El Observatorio permite transformar reclamos o incidencias aisladas en información territorial útil para la toma de decisiones. La lectura conjunta de categorías, estados y distribución espacial permite detectar hotspots urbanos, priorizar intervenciones y sostener seguimiento mensual.</p>

        <h2>7. Recomendaciones</h2>
        <ul>
            <li>Sostener carga mensual normalizada.</li>
            ...
            <li>Revisar periódicamente categorías y estados para evitar dispersión de datos.</li>
            <li>Priorizar sectores con clasificación Prioridad 1 - Crítica.</li>
            <li>Utilizar la serie histórica como insumo para planificación y comunicación institucional.</li>
        </ul>

        <h2>8. Cierre</h2>
        <p>El valor central del Observatorio Territorial EGS no reside únicamente en visualizar datos, sino en convertir registros operativos en indicadores, prioridades y evidencia territorial para la gestión.</p>
    </body>
    </html>
    """
    return html
...
def generar_informe_txt(df, selected_months):
    ...
    criticidad = round(df["criticidad"].mean(), 1) if total else 0
    periodo = ", ".join(selected_months) if selected_months else "Todos los registros"
    return f"""EGS | INFORME TÉCNICO TERRITORIAL
...
- Prioridad media EGS: {criticidad}

Resultados:
La categoría predominante fue {cat_top}. El sector con mayor concentración territorial fue {barrio_top}.

Metodología:
El Índice Territorial EGS combina cantidad de incidencias, criticidad promedio y casos activos:
Índice EGS = incidencias × 1,5 + criticidad promedio × 2 + casos activos × 1,4.
...
Cierre:
El Observatorio Territorial EGS transforma datos operativos en indicadores y prioridades para la toma de decisiones.
"""
...
def excel_download(df):
    ...
    def generar_informe_tecnico(df, selected_months, usuario_actual="EGS"):

        total = len(df)
        activos = int(df["activo"].sum()) if total else 0
        cerrados = total - activos

        tasa_estado, tasa_resolucion = semaforo_gestion(df)
        ...
        barrio_top = ranking.iloc[0]["barrio"] if not ranking.empty else "-"

        indice_top = ranking.iloc[0]["indice_egs"] if not ranking.empty else "-"

        categoria_top = df["categoria"].value_counts().idxmax() if total else "-"

        informe = f"""
    EGS | INFORME TÉCNICO TERRITORIAL

    Sistema: Observatorio Territorial EGS
    Dominio previsto: observatorio.egsestudiodegestionsistemas.com
    Fecha de emisión: {datetime.now().strftime("%d/%m/%Y")}
    Usuario emisor: {usuario_actual}
    Período analizado: {periodo}

    1. OBJETO DEL INFORME

    Analizar incidencias territoriales registradas en el sistema y generar indicadores para apoyo a la toma de decisiones.
    ...
    Tasa de resolución: {round(tasa_resolucion,1)} %
    Criticidad promedio: {criticidad}

    3. PRINCIPALES HALLAZGOS

    Categoría predominante: {categoria_top}
    Barrio con mayor índice territorial: {barrio_top}
    Índice EGS máximo: {indice_top}

    4. CONCLUSIÓN

    La información analizada evidencia patrones territoriales relevantes que requieren seguimiento sistemático mediante indicadores estratégicos y planificación operativa.
...
with st.sidebar:
    ...
    st.caption("Uso interno EGS + equipo de gestión. Carga CSV/Excel mensual y actualización automática.")
...
<div class="egs-header">
    ...
    <p class="small-note">Despliegue previsto: observatorio.egsestudiodegestionsistemas.com</p>
</div>
""", unsafe_allow_html=True)
...
if module == "Dashboard":
    ...
    cards = [
        ...
        (c4, "Prioridad media EGS", criticidad, "Escala técnica interna"),
    ...
    with col1:
        fig = px.bar(dff.groupby("categoria").size().reset_index(name="cantidad"), x="categoria", y="cantidad", title="Incidencias por categoría", text="cantidad")
    ...
    with col3:
        fig = px.line(df.groupby("mes").size().reset_index(name="cantidad"), x="mes", y="cantidad", markers=True, title="Evolución mensual de incidencias")
        st.plotly_chart(fig, use_container_width=True)
    with col4:
    ...
    st.write("Subí el CSV o Excel recibido. El sistema normaliza columnas, guarda copia histórica y actualiza todo el observatorio.")
    ...
    if uploaded:
        ...
        if st.button("Incorporar al histórico"):
            ...
            combined = combined.drop_duplicates(subset=["fecha", "categoria", "descripcion", "direccion"], keep="last")
            save_base(combined)
            st.success("Carga mensual incorporada. El tablero ya quedó actualizado.")
            st.rerun()
    ...
    if map_df.empty:
        ...
        fig = px.scatter_map(map_df, lat="latitud", lon="longitud", color="categoria", size="criticidad", hover_name="categoria", hover_data=["descripcion", "direccion", "barrio", "estado", "criticidad"], zoom=12, height=650, title="Distribución georreferenciada de incidencias")
        fig.update_layout(map_style="open-street-map", margin={"r": 0, "t": 40, "l": 0, "b": 0})
        st.plotly_chart(fig, use_container_width=True)
        fig2 = px.density_map(map_df, lat="latitud", lon="longitud", z="criticidad", radius=28, zoom=12, height=600, title="Mapa de densidad territorial")
        fig2.update_layout(map_style="open-street-map", margin={"r": 0, "t": 40, "l": 0, "b": 0})
    ...
    st.write("El índice combina cantidad de incidencias, criticidad promedio y casos activos.")
    st.dataframe(ranking, use_container_width=True)
    if not ranking.empty:
        fig = px.bar(ranking.head(10), x="indice_egs", y="barrio", color="nivel_prioridad", orientation="h", title="Ranking de prioridad territorial")
        st.plotly_chart(fig, use_container_width=True)
        top = ranking.iloc[0]
        st.markdown(f"""<div class="alert-box"><b>Lectura automática EGS:</b><br>El sector con mayor prioridad territorial es <b>{top['barrio']}</b>, con {int(top['incidencias'])} incidencias, criticidad promedio {round(top['criticidad_promedio'], 1)} e índice EGS {top['indice_egs']}.</div>""", unsafe_allow_html=True)

elif module == "Hotspots urbanos":
    st.markdown("## Hotspots urbanos")
    st.write("Detección automática de concentración territorial y categorías dominantes.")
    col1, col2 = st.columns(2)
    with col1:
        ...
        if not barrios.empty:
            ...
            st.plotly_chart(fig, use_container_width=True)
    with col2:
        st.markdown("### Calles o direcciones recurrentes")
    ...
    st.write("Evaluación visual de desempeño por categoría y sector.")
    ...
    if not dff.empty:
        sem_cat = dff.groupby("categoria").agg(total=("categoria", "count"), activos=("activo", "sum")).reset_index()
        sem_cat["resueltos"] = sem_cat["total"] - sem_cat["activos"]
        sem_cat["tasa_resolucion"] = round((sem_cat["resueltos"] / sem_cat["total"]) * 100, 1)
        sem_cat["semaforo"] = sem_cat["tasa_resolucion"].apply(lambda x: "🟢 Excelente" if x >= 80 else "🟡 A mejorar" if x >= 50 else "🔴 Crítico")
    ...
    st.markdown("## Informe técnico EGS")
    ...
    informe_html = generar_informe_tecnico(dff, selected_months)

    st.text_area("Vista rápida del informe", informe_txt, height=420)

    col1, col2 = st.columns(2)
    with col1:
        st.download_button("Descargar informe técnico HTML", informe_html.encode("utf-8"), file_name="informe_tecnico_egs.html", mime="text/html")
    with col2:
        st.download_button("Descargar informe técnico TXT", informe_txt.encode("utf-8"), file_name="informe_tecnico_egs.txt", mime="text/plain")

elif module == "Exportar":
    st.markdown("## Exportación de datos")
    st.download_button("Descargar base filtrada CSV", dff.to_csv(index=False).encode("utf-8"), file_name="base_filtrada_egs.csv")
    st.download_button("Descargar informe Excel", excel_download(dff), file_name="observatorio_territorial_egs.xlsx")

elif module == "Guía de columnas":
    st.markdown("## Guía para cargar datos reales")
    guia = pd.DataFrame({
        "columna_recomendada": ["fecha", "categoria", "descripcion", "direccion", "barrio", "latitud", "longitud", "estado", "prioridad", "fecha_resolucion", "criticidad"],
        "ejemplo": ["2026-06-15", "Baches", "Bache en esquina", "Av. Libertador 1200", "Centro", "-31.4275", "-62.0821", "Pendiente", "Alta", "2026-06-20", "8"],
        "obligatoria": ["Sí", "Sí", "No", "No", "No", "No", "No", "No", "No", "No", "No"]
    })
    st.dataframe(guia, use_container_width=True)
    plantilla = pd.DataFrame(columns=guia["columna_recomendada"])
    st.download_button("Descargar plantilla Excel", excel_download(plantilla), file_name="plantilla_carga_egs.xlsx")
